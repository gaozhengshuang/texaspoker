...
class GamblingPanelMoveSupport extends BaseGamblingPanelSupport
{
	 ...
	 * 忽略触发列表
	 */
	private _ignoreList: Array<egret.DisplayObject>
	/**
	 * 禁止滑动
	 */
	private _stopMove: boolean;
	...
	public onEnable()
	{
		...
		if (!this._ignoreList)
		{
			this._ignoreList = new Array<egret.DisplayObject>();
			this._ignoreList.push(this.target.audioRecordBtn);
		}
		for (let obj of this._ignoreList)
		{
	...
	*/
	...
	private ignoreListBegin(event: egret.TouchEvent)
	{
		this._stopMove = true;
	}

	private ignoreListEnd(event: egret.TouchEvent)
	{
		this._stopMove = false;
	}
}