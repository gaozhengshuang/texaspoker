module app {
    export class ClientNet {
        ...
        private onNotifyMessage(mainId: number, cmdDataBA: egret.ByteArray) {
            ...
            game.NotificationCenter.postNotification(protoData.Name, decoded);
        }

        //添加链接打开侦听，连接成功会调用此方法
        private onSocketOpen() {
            this.isConnect = true;
            game.NotificationCenter.postNotification(ClientNet.SOCKET_CONNECT_SUCCESS);
        }

        //添加链接关闭侦听，手动关闭或者服务器关闭连接会调用此方法
        private onSocketClose() {
            this.isConnect = false;
            game.NotificationCenter.postNotification(ClientNet.SOCKET_CONNECT_CLOSE);
        }
