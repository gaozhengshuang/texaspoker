...
/**
 * 获取path子级文件路径添加到list并返回
 */
exports.getChildFilePath = function(path, suffixs)
{
    var list = [];
    exports.refChildFilePath(path, suffixs, list);
    return list;
}
/**
...
/**
 * 获取path下所有指定扩展名的文件路径添加到list中，list不能为空
 * @param path
 * @param suffixs
 * @param list
 */
exports.refChildFilePath = function (path, suffixs, list)
{
    if(FS.existsSync(path))
    {
        var files = FS.readdirSync(path);
        ...
        {
            var currentPath = Path.join(path, file);
            if(!FS.statSync(currentPath).isDirectory() && suffixs && suffixs.indexOf(Path.extname(currentPath)) >= 0)
            {
                list.push(currentPath);
            }
        }, this);
